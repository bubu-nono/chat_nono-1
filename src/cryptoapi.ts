...
export async function ticker(message: Message, text: string): Promise<string> {
    try {
        ...
        const pre: string | null = await Redis.getSetEx(`${key_crypto}${w.symbol}`, `${w.price}`, 86400);
...
export async function ticker2(text: string): Promise<string> {
    try {
        const code = `${text.substring(1).toUpperCase()}USDT`;
        // 👇️ const response: Response // 美国：api.binance.us，其他：api.binance.com
        const response = await fetch(`https://api.binance.us/api/v3/ticker/price?symbol=${code}`, {
            method: 'GET',
            headers: {
                Accept: 'application/json',
            },
...
export async function kline(message: Message, text: string): Promise<string> {
    try {
